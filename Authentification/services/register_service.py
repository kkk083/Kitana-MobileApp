...
class RegisterService:
    ...
    @staticmethod
    def register_user(nom: str, email: str, password: str, role: str = "etudiant") -> Tuple[bool, Optional[User], str]:
        """
        ...
        Args:
            ...
            role: Rôle de l'utilisateur (default: 'etudiant')
    ...
    def _insert_user(user: User) -> Optional[int]:
        ...
        try:
            with DatabaseConnection(commit=True) as cursor:
                query = """
                    INSERT INTO users (nom, email, password, role, date_creation)
                    VALUES (%s, %s, %s, %s, %s)
                    RETURNING id_user
                """
